...
cd /data/vaultwarden
docker compose stop
cp /tmp/vault-restore/var/backups/vaultwarden/stage/data/db.sqlite3 ./data/db.sqlite3
if [ -d /tmp/vault-restore/var/backups/vaultwarden/stage/data/attachments ]; then
  rsync -a /tmp/vault-restore/var/backups/vaultwarden/stage/data/attachments/ ./data/attachments/
fi
chown -R root:root ./data
docker compose start
```

A vault with no attachments has no `attachments` directory, which is why the copy is guarded.
