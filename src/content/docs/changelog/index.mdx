...
import { changelogFeedEntries, publicationDateAtUtcMidnight } from '../../../data/changelog-feed';

export const formatFeedDate = (date) =>
  new Intl.DateTimeFormat('en-CA', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    timeZone: 'UTC',
  }).format(publicationDateAtUtcMidnight(date));
...
{changelogFeedEntries.map((entry) => (

  <p>
    <a href={`/changelog/#${entry.slug}`}>{entry.title}</a> ({formatFeedDate(entry.pubDate)})
  </p>
))}
...
## Platform & services {/* changelog-id: platform-services */}

Updates to the Public Cloud platform and managed services.

### Up to 8 subnets per VPC (September 7, 2026) {/* changelog-id: vpc-subnet-limit */}

Each VPC supports up to 8 subnets, up from 3. Existing VPCs get the new limit with no changes on
your side. If a workload needs more than 8 subnets in a single VPC, raise a support ticket from the
console. We will review the request. See [Add Subnet](/public-cloud/networking/vpc/add-subnet).

### Object storage endpoint DNS resolution from VPCs (September 6, 2026) {/* changelog-id: object-storage-vpc-dns-resolution */}

Fixed DNS configuration for newly created VPCs. Workloads in these VPCs now resolve the object
storage endpoint.

### Kubernetes 1.37 is available (September 6, 2026) {/* changelog-id: kubernetes-1.37 */}

New managed clusters can run 1.37.0 or the refreshed patch releases 1.36.4 (the default), 1.35.8,
and 1.34.11. At release, clusters already running 1.36.1, 1.35.1, or 1.34.3 remained on those
versions until an upgrade. Those earlier patch releases are no longer offered for new clusters.
Upgrade one minor version at a time, or apply a patch upgrade within the same minor version. Do this
from the [Cluster Overview](/public-cloud/kubernetes/cluster-overview) page. These versions are
available in both regions. See [Create Kubernetes Cluster](/public-cloud/kubernetes/create-cluster)
for the full version list and lifecycle information.

### Postpaid billing is available (September 1, 2026) {/* changelog-id: postpaid-billing */}

Choose Postpaid during signup for eligible hourly and monthly services, then pay invoices with a
credit card saved through Stripe. Choose Prepaid or Postpaid during signup. After you create your
account, it remains in that billing mode for the life of the account. See
[Billing](/public-cloud/billing).

### Intel compute in Montréal (YUL) {/* changelog-id: intel-compute-yul */}

General-purpose `ci2` plans from 1 to 96 GB of RAM, memory-optimized `cim2` plans from 8 to 64 GB of
RAM, custom Intel configurations, and Intel-backed Kubernetes node capacity. See
[Instance types](/public-cloud/compute/instance-types).

### Outbound SMTP on Port 25 {/* changelog-id: smtp-port-25 */}

Outbound SMTP on port 25 is now blocked by default on all compute instances to protect platform-wide
sender reputation. Ports 465 and 587 remain open for authenticated email. Verified customers can
request access through support. See
...
## Marketplace {/* changelog-id: marketplace */}
...
### v0.0.29: September 7, 2026 {/* changelog-id: cli-v0.0.29 */}

**Complete volume listings and clearer VPC quota errors.** See the
[v0.0.29 release](https://github.com/zsoftly/zcp-cli/releases/tag/v0.0.29) on GitHub.

- **Volume listings now retrieve every page.** The CLI and its Go SDK no longer miss volumes beyond
  the API's first results page. Pagination anomalies return explicit errors instead of partial or
  duplicated results.
- **VPC subnet-limit failures explain the next step.** When `zcp network create --vpc` reaches the
  default limit of eight subnets, it directs you to request a quota increase through support. Run
  the command again after the increase is applied. See the
  [CLI reference](/public-cloud/cli/reference).

### v0.0.28: September 7, 2026 {/* changelog-id: cli-v0.0.28 */}

**Security fixes, working backups, and a public-IP-first `instance ssh`.** This release also ships
everything prepared for v0.0.27, which was never published. Upgrading from v0.0.26 picks up both
sets of changes. See the [v0.0.28 release](https://github.com/zsoftly/zcp-cli/releases/tag/v0.0.28)
on GitHub.

- **`--debug` output no longer leaks your API token.** Debug output and error messages now redact
  the token, the `password` field, and other credential fields. This includes messages built from
  responses the CLI cannot parse. We updated `golang.org/x/crypto` and the Go toolchain to close two
  SSH denial-of-service advisories. Rotate your token if you shared `--debug` output before this
  release.
- **Backups work again.** `backup list` decodes the scheduled hour whether the API sends it as a
  number or a string. `vm-backup delete` now works: it submits a cancellation request instead of the
  `DELETE` request the API always rejected. `--interval` accepts only `dailyAt` or `hourlyAt`.
  `backup list` and `vm-backup list` show clearer columns.
- **`instance ssh` prefers the public IP.** It connects over the public IP when one is attached and
  falls back to the private IP otherwise. New `--use-public` and `--use-private` flags force the
  choice.
- **Output fixes**: `firewall list` now shows each rule's state. `dns show` prints `-` instead of a
  fabricated status. `autoscale policy delete` and `autoscale condition delete` print the numeric ID
  correctly.
- **From v0.0.27**: `instance create` supports the `Vpc` network type and can attach existing
  networks with `--networks`. `ip static-nat enable` now requires `--network`. `volume attach` and
  `volume detach` show the API's status instead of a blank row.
- **Known limitation**: `object-storage bucket encryption enable` is disabled until the region's
  gateway supports SSE-S3 default encryption. `status` and `disable` still work.

### v0.0.26: July 19, 2026 {/* changelog-id: cli-v0.0.26 */}
...
### v0.0.25: July 18, 2026 {/* changelog-id: cli-v0.0.25 */}
...
### v0.0.24: July 16, 2026 {/* changelog-id: cli-v0.0.24 */}
...
### v0.0.17: June 17, 2026 {/* changelog-id: cli-v0.0.17 */}
...
## Terraform / OpenTofu provider {/* changelog-id: terraform-opentofu */}
...
### v0.2.0: September 7, 2026 {/* changelog-id: terraform-v0.2.0 */}

**Object storage configuration, richer instance networking, and volume lookups.** See the
[v0.2.0 release](https://github.com/zsoftly/terraform-provider-zcp/releases/tag/v0.2.0) and
[tagged changelog](https://github.com/zsoftly/terraform-provider-zcp/blob/v0.2.0/CHANGELOG.md) for
the complete change list.

- **Manage object storage bucket settings as code.** New `zcp_object_storage_bucket_versioning`,
  `zcp_object_storage_bucket_policy`, `zcp_object_storage_bucket_tagging`,
  `zcp_object_storage_bucket_lifecycle`, and `zcp_object_storage_bucket_cors` resources manage the
  respective S3-compatible gateway settings. The provider obtains gateway credentials internally and
  does not expose them as resource attributes.
- **Attach instances to VPCs and multiple networks.** `zcp_instance` now supports the `Vpc` network
  type and multiple networks, matching the instance-create capabilities in CLI v0.0.27.
- **Look up volumes and attached disks.** The new `zcp_volume` data source looks up an existing
  volume. `data.zcp_instance` now exposes `root_volume` and `volumes`.
- **Read volume backups again.** `zcp_volume_backup` now decodes the platform backup-list shape
  correctly.
- **Destroy VM backup schedules.** `zcp_vm_backup` destroys schedules through the billing
  cancellation workflow instead of a rejected direct delete.
- **Validate backup intervals before apply.** `zcp_vm_backup` and `zcp_volume_backup` accept only
  `dailyAt` or `hourlyAt` for `interval`, matching the platform.
- **Fail fast for identifiable VPC public IPs.** When `zcp_firewall_rule` identifies a VPC public
  IP, it fails immediately instead of polling for a rule the platform will not apply.
- **Fix VPC public-IP ordering guidance.** `zcp_ip_address` explains that a VPC network tier must
  exist first and indicates when to add `depends_on`.
- **Complete volume listings.** The provider now uses the `zcp` CLI SDK v0.0.29, which retrieves
  every page in a volume list.
...
Run `terraform init -upgrade` for Terraform or `tofu init -upgrade` for OpenTofu. If OpenTofu
reports no matching version, wait until its registry lists v0.2.0, then retry.
