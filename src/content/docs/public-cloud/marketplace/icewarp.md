...
| `ICEWARP_USE_HTTPS`            | `0` or `1`; enables HTTPS after DNS and TLS are configured      |
...
Do not submit administrator credentials to a public HTTP URL. For direct public access, first point
`ICEWARP_PUBLIC_HOSTNAME` DNS to the VM, configure a valid TLS certificate, set
`ICEWARP_USE_HTTPS=1`, then use `https://<your-public-hostname>/` or
