...
Log in with username `admin` and the password from `/etc/dns/credentials.txt`. Do not submit
administrator credentials through public HTTP.
...
| 22   | TCP      | SSH                                              |
| 53   | TCP/UDP  | DNS                                              |
| 5380 | TCP      | Admin web console (access through an SSH tunnel) |
...
recursion policy or configure a specific trusted recursion ACL. Restrict port `5380` to trusted
access and use an SSH tunnel for the admin console.
